...
Each event is a JSON-compatible object with canonical fields:
...
1. Call `init()` from `@twilic/core` before encode/decode.
2. Encode header and body separately with stateless Twilic encode.
3. Preserve event order in the body array.

## MIME type

`application/vnd.twilic.ai+twai`

## Reference implementation

See `@twilic/ai` (`packages/core`) for `encodeTwai`, `decodeTwai`, `readSession`, and `writeSession`.
